            ...
            return (
              <Link
                ...
                className={`px-4 py-2 rounded-sm text-[10px] uppercase tracking-[0.16em] transition-all ${active
                  ? "bg-ifx-gold/12 border border-ifx-gold/35 text-ifx-gold shadow-[0_0_20px_rgba(212,175,55,0.15)]"
        ...
        {posts.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
            {posts.map((post, idx) => (
              ...
              >
                <article
                  className="glass-premium relative overflow-hidden h-full border border-white/8 p-7 transition-all duration-500 hover:-translate-y-2 hover:border-ifx-gold/40 hover:shadow-[0_20px_50px_rgba(212,175,55,0.12)]"
                >
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-ifx-gold/0 via-ifx-gold/60 to-ifx-gold/0 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <p className="text-[10px] uppercase tracking-[0.14em] text-ifx-gold mb-3">
                    {post.category}
                  </p>
                  <h2 className="font-serif text-2xl leading-snug group-hover:text-ifx-gold transition-colors">{post.title}</h2>
                  ...
                  <div className="mt-8 flex items-center justify-between">
                    ...
                    <span className="text-[10px] uppercase tracking-[0.16em] text-ifx-gold/70 group-hover:text-ifx-gold">
                      Read Analysis →
                    </span>
                  </div>
                </article>
