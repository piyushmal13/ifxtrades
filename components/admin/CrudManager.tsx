                  ...
                  <div className="relative group w-full h-32 bg-white/5 border border-white/15 rounded-sm overflow-hidden flex items-center justify-center">
                    {form[field.name] ? (
                      ...
                      <div className="text-white/20 flex flex-col items-center gap-1">
                        ...
                        <span className="text-[10px] uppercase">No Image Set</span>
                      </div>
                    )}
                  </div>
                  <input
                    type="text"
                    className="bg-white/5 border border-white/15 rounded-sm px-3 py-2 text-sm text-white w-full"
                    placeholder="Enter Image URL (e.g. Supabase Storage link)"
                    value={form[field.name]}
                    required={field.required}
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, [field.name]: event.target.value }))
